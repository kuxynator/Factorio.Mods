...
function data_util.tech_remove_prerequisites (prototype_name, prerequisites)
  local prototype = data.raw.technology[prototype_name]
  if not prototype then return end
  if not prototype.prerequisites then return end
  for _, new_prerequisite in pairs(prerequisites) do
...
function data_util.transition_icons(icon_from, icon_to)
  if type(icon_to) == "string" then
    return {
      { icon = "__space-exploration__/graphics/blank.png", scale = 1, shift = {0, 0}, icon_size = 32 }, -- to lock scale
      { icon = icon_from, scale = 0.66, shift = {8, -8}, icon_size = 32 },
      { icon = icon_to, scale = 0.66, shift = {-8, 8}, icon_size = 32 },
      { icon = "__space-exploration__/graphics/icons/transition-arrow.png", scale = 1, shift = {0, 0}, icon_size = 32 }, -- to overlay
    }
  else
    local icons = {
      { icon = "__space-exploration__/graphics/blank.png", scale = 1, shift = {0, 0}, icon_size = 32 }, -- to lock scale
    ...
    table.insert(icons, { icon = "__space-exploration__/graphics/icons/transition-arrow.png", scale = 1, shift = {0, 0}, icon_size = 32 })
    return icons
  end
end
