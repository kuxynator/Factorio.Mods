...
Spaceship.engines = {
  [mod_prefix .. "spaceship-rocket-engine"] = { name = mod_prefix .. "spaceship-rocket-engine", thrust = 100, max_energy = 1837 },
  [mod_prefix .. "spaceship-antimatter-engine"]= { name = mod_prefix .. "spaceship-antimatter-engine", thrust = 500, max_energy = 18370 }
}
...
Spaceship.names_spaceship_bulkheads = {
  ...
  Spaceship.name_spaceship_rocket_engine,
  Spaceship.name_spaceship_antimatter_engine
}
...
function Spaceship.launch(spaceship)
  ...
  if not ship_surface then
    ...
    ship_surface.daytime = 0.4 -- dark but not night
...
function Spaceship.land_at_position(spaceship, position)
  if spaceship.own_surface_index and spaceship.near and spaceship.near.type == "zone" and spaceship.known_tiles then
    ...
    for _, particle in pairs(Spaceship.particles) do
      if particle.graphic_name then
        table.insert(particle_names, particle.graphic_name)
      end
      if particle.projectile_name then
        table.insert(particle_names, particle.projectile_name)
      end
      if particle.targetable_name then
        table.insert(particle_names, particle.targetable_name)
      end
    end

    local particles = ship_surface.find_entities_filtered{name = particle_names}
    ...
    for _, character in pairs(characters) do
      teleport_character_to_surface(character, target_surface, {
    ...
    if spaceship.near and spaceship.near.type == "spaceship" then
      ...
      if othership then
        if not player.character then
          player.print({"space-exploration.no-character-to-board"})
        else
          ...
          local playerdata = get_make_playerdata(player)
          playerdata.position = {x = player.position.x, y = player.position.y + 1}
          playerdata.set_position = playerdata.position
          playerdata.velocity = {x=0,y=-0.1}
    ...
    if spaceship then
      ...
      if spaceship.space_distortion > 0.05 then
        ...
        if spaceship.planet_gravity_well > 0 then
          ...
          root["planet-gravity"].caption = {"space-exploration.spaceship-location-star-gravity-well", string.format("%.2f", spaceship.planet_gravity_well * Zone.travel_cost_planet_gravity)}
        else
          root["planet-gravity"].caption = ""
        end
      end
      ...
      elseif spaceship.near and spaceship.near.type == "spaceship" and spaceship.destination
        and spaceship.destination.type == "spaceship" and spaceship.destination.index == spaceship.near.index then
          ...
          if not root["action-flow"]["board"] then
            root["action-flow"].clear()
            root["action-flow"].add{ type="button", name="board", caption={"space-exploration.spaceship-button-board", othership.name}, style="confirm_button"}
          end
...
function Spaceship.on_player_opened_changed(event)
  ...
  if opened_new and opened_new.valid and opened_gui_type_new == defines.gui_type.entity and opened_new.name == Spaceship.name_spaceship_console then
    Spaceship.gui_open(player, Spaceship.from_entity(opened_new))
    player.opened = nil
  else
...
Event.addListener("on_player_opened_changed", Spaceship.on_player_opened_changed, true)

function Spaceship.on_entity_created(event)  local entity
...
function Spaceship.on_removed_entity(event)
  if event.entity and event.entity.valid and event.entity.surface
   and Util.table_contains(Spaceship.names_spaceship_bulkheads, event.entity.name) then
...
function Spaceship.get_distance_to_destination(spaceship)
  if (not spaceship.destination) or Spaceship.is_at_destination(spaceship) then
    return 0
  end
...
function Spaceship.activate_engines(spaceship)
  ...
  if spaceship.engines then
      for _, engine in pairs(spaceship.engines) do engine.active = true end
  end
end
...
function Spaceship.deactivate_engines(spaceship)
  Spaceship.find_own_surface_engines(spaceship)
  if spaceship.engines then
      for _, engine in pairs(spaceship.engines) do engine.active = false end
  end
end
         ...
         if spaceship.speed > 0.001 then
            ...
            if particle.targetable and particle.destroys_floor then
              ...
              if spaceship.known_tiles[x] and spaceship.known_tiles[x][y] then
                ...
                Spaceship.get_own_surface(spaceship).set_tiles({{name=name_space_tile, position = {x,y}}})
                particle.targetable.damage(50, "neutral", "explosion")
                Spaceship.start_integrity_check(spaceship)
              end
            end
...
function Spaceship.set_light(spaceship, surface)
    ...
    local light_percent = math.min(1, 0.1 + spaceship.star_gravity_well/7.5)
...
function Spaceship.move_to_destination(spaceship)
  ...
  if spaceship.near and not Spaceship.is_at_destination(spaceship) then
  ...
  if spaceship.own_surface_index or (spaceship.console and spaceship.console.valid) then
    ...
    if spaceship.own_surface_index then
      ...
      if spaceship.integrity_valid
        ...
        and not Spaceship.is_at_destination(spaceship) then
        ...
        if Spaceship.is_at_destination(spaceship) then
          spaceship.speed = 0
        ...
        local set_tiles = {}
          ...
          for _, tile in pairs(all_tiles) do
            if not (spaceship.check_tiles[tile.position.x] and spaceship.check_tiles[tile.position.x][tile.position.y]) then
              ...
              table.insert(set_tiles, {name = name_space_tile, position = tile.position, stack = stack})
          ...
          if #set_tiles > 0 then
            ...
            --  surface.create_entity{name = "item-on-ground", stack = tile.stack, position = {tile.position.x+0.5, tile.position.y+0.5}}
            --end
          end
        end
      end
              ...
              if spaceship.own_surface_index and spaceship.is_moving then
                ...
                if support <= 6 then -- has a chance to be removed
                  ...
                  if support - math.random(2) <= 4 then
                    ...
                    if remove then
                      table.insert(set_tiles, {name = name_space_tile, position = {x,y}})
                    end
                  end
                end
              end
