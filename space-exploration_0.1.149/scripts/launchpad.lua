...
function Launchpad.get_cargo_loss(force) -- percentage of cargo lost
...
function Launchpad.get_survivability_loss(force) -- chance to fail to land on rocket launch pad
  local multiplier = 1
  local i = 1
        ...
        if (zone.type == "orbit" and zone.parent.name == "Nauvis") then
          if (global.forces[force_name].cargo_rockets_launched or 0) == 0 and global.forces[force_name].nauvis_satellite then
            locked_destination.position = {
              x = global.forces[force_name].nauvis_satellite.x + satellite_blueprint.landing_offset[1] - satellite_blueprint.center[1],
              y = global.forces[force_name].nauvis_satellite.y + satellite_blueprint.landing_offset[2] - satellite_blueprint.center[2]
            }
          end
        end
        return locked_destination
...
function Launchpad.update_combinator(struct, main_inv)
  if struct.combinator then
    ...
    if struct.rocket_sections >= Launchpad.rocket_sections_per_rocket and struct.crew_capsules >= Launchpad.crew_capsules_per_rocket then
      comb.set_signal(4, {signal={type="item", name=Launchpad.name_rocket_launch_pad_silo_dummy}, count=1})
    else
      comb.set_signal(4, {signal={type="item", name=Launchpad.name_rocket_launch_pad_silo_dummy}, count=0})
    end
...
function Launchpad.launch(struct, skip_prep, allow_non_empty)
  ...
  if destination then
    ...
    if lua_fuel_consumed < required_fuel then
      ...
      fluidbox.amount = fluidbox.amount - tank_fuel_consumed
      struct.tank.fluidbox[1] = fluidbox
  ...
  if tick_task.launch_timer then
    ...
    if not tick_task.land_timer then
      ...
      if is_landingpad then
        local error_chance = Launchpad.get_survivability_loss(game.forces[tick_task.force_name])
      ...
      local cargo_loss = Launchpad.get_cargo_loss(game.forces[tick_task.force_name])
      if is_landingpad then
...
function Launchpad.on_player_opened_changed(event)
  ...
  if opened_new and opened_new.valid and opened_gui_type_new == defines.gui_type.entity and opened_new.name == Launchpad.name_rocket_launch_pad then
    Launchpad.gui_open(player, Launchpad.from_entity(opened_new))
  else
...
Event.addListener("on_player_opened_changed", Launchpad.on_player_opened_changed, true)
