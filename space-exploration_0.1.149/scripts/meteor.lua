...
function Meteor.on_player_opened_changed(event)
  ...
  --[[
  if opened_new and opened_new.valid and opened_gui_type_new == defines.gui_type.entity and opened_new.name == Meteor.name_meteor_point_defence_container then
    if not playerdata.meteor_point_defence_radius then
      playerdata.meteor_point_defence_radius = rendering.draw_circle{
        color = {r = 0, g = 10, b = 0, a = 0.01},
        players = {player},
        radius = Meteor.point_defence_range,
        filled = true,
...
Event.addListener("on_player_opened_changed", Meteor.on_player_opened_changed, true)

function Meteor.on_player_selected_changed(event)
  local player = event.player
  local selected_new = event.selected_new
  local selected_old = event.selected_old
  ...
  --[[
  if selected_new and selected_new.valid and selected_new.name == Meteor.name_meteor_point_defence_container then
    if not playerdata.meteor_point_defence_radius then
      playerdata.meteor_point_defence_radius = rendering.draw_circle{
        color = {r = 0, g = 10, b = 0, a = 0.01},
        players = {player},
...
Event.addListener("on_player_selected_changed", Meteor.on_player_selected_changed, true)
...
function Meteor.on_removed_entity(event)
  if event.entity and event.entity.valid and event.entity.surface then
    if event.entity.name == Meteor.name_meteor_defence_container then
    ...
    if event.entity.name == Meteor.name_meteor_point_defence_container then
       local entity = event.entity
